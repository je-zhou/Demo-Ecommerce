...
export function findPrice(
  ...
  if (product.pricingMatrix) {
    const newPriceCombo = sortedPricingMatrix.find((pc) =>
      sameVariantCombo(pc.values, selectedVariants, variablePricingIds)
    );
    if (newPriceCombo) {
...
export function sortPricingMatrix(
  pricingMatrix: ProductPricingMatrix[] | undefined,
  variablePricingIds: string[]
) {
  if (pricingMatrix) {
...
export function sameVariantCombo(
  variants1: { [key: string]: string },
  variants2: { [key: string]: string },
  variablePricingIds: string[]
) {
...
export function getVariablePrice(
  selectedVariants: { [key: string]: string },
  pricingMatrix: ProductPricingMatrix[],
  variablePricingIds: string[]
) {
  const sortedPricingMatrix = sortPricingMatrix(
    pricingMatrix,
    variablePricingIds
  );
...
export function calculateProductShipping(
  ...
  const productShipping = product.shipping.find((s) =>
    s.regions.some((r) => r.id === countryCode)
  );
  ...
  if (productShipping) {
    ...
    let itemPrice = product.pricingMatrix
      ? getVariablePrice(
          product.selectedVariants as any,
          product.pricingMatrix as any,
          variablePricingIds
        ) ?? product.price
      : product.price;
    ...
    if (varShipping && productShipping.isVariablePrice) {
      let test =
        varShipping.type === "Price"
          ? Number(itemPrice)
          : Number(product.weight) ?? 0;

      const sortedConditions = varShipping.conditions.sort(
